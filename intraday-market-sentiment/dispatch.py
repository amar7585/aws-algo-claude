"""
Handing the snapshot to strategy-orchestrator.

WHY THIS IS AN INVOKE AND NOT A SCHEDULE. The orchestrator needs the snapshot,
and the snapshot exists only once this function has written it. A cron on the
orchestrator's side would have to guess how long that takes, read the row back
...
downstream playbook was slow, and the orchestrator has its own log group with
error-notifier watching it. What is reported here is only whether the invoke
was accepted.

UNSET MEANS OFF. With no ORCHESTRATOR_FUNCTION_NAME configured this does
...
setting one environment variable once the orchestrator exists and this
...
from config import ORCHESTRATOR_FUNCTION_NAME, ORCHESTRATOR_INVOCATION_TYPE
...
def dispatch_snapshot(row, instrument, client=None):
    """
    Invoke strategy-orchestrator with the row just written.

    Returns the function name on success, or None when no orchestrator is
    ...
    if not ORCHESTRATOR_FUNCTION_NAME:
        logger.info(
            "no ORCHESTRATOR_FUNCTION_NAME set - the snapshot is written and "
    ...
    response = client.invoke(
        ...
        InvocationType=ORCHESTRATOR_INVOCATION_TYPE,
    ...
    if status not in (200, 202):
        raise RuntimeError(
            f"invoking {ORCHESTRATOR_FUNCTION_NAME} returned StatusCode "
            f"{status} - the snapshot is written but no strategy was reached"
        )
    if response.get("FunctionError"):
        raise RuntimeError(
            f"{ORCHESTRATOR_FUNCTION_NAME} reported "
            f"{response['FunctionError']}"
        )
    logger.info(
        "dispatched snapshot %s to %s (%s)",
        row["snapshot_ts"], ORCHESTRATOR_FUNCTION_NAME, ORCHESTRATOR_INVOCATION_TYPE,
    )
    return ORCHESTRATOR_FUNCTION_NAME
