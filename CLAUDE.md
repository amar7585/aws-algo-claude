...
1. **No compiled dependencies.** Pure Python only. This is what keeps everything
   a zip-packaged Lambda instead of a container. Do not reintroduce `pandas`,
   `psycopg2`, or the `dhanhq` SDK — an earlier iteration moved the whole
   project to ECS Fargate because of exactly those three, and was reverted. If
   something seems to need a C extension, replace it or rethink the component.
...
- **Handler must be `handler.lambda_handler`.** Set under Code tab → Runtime
  settings → Edit, *not* Configuration → General configuration. A mismatch
  gives `Handler 'handler' missing on module 'handler'`.
